...
// Возвращает остатки на складе и количество списанного в производство
// для списка материалов одним пакетным запросом.
//
// Параметры:
//   Ссылка - ДокументСсылка - Ссылка на документ-основание (Рецептуры)
//   СписокМатериалов - Массив из СправочникСсылка.НоменклатураМатериалы - Список материалов
//
// Возвращаемое значение:
//   Соответствие - Ключ: СправочникСсылка.НоменклатураМатериалы,
//                  Значение: Структура с полями Остаток (Число) и ВПроизводстве (Число)
Функция ОстаткиИВПроизводстве(Ссылка, СписокМатериалов) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ОстаткиМатериаловОстатки.Наименование КАК Наименование,
		|	ОстаткиМатериаловОстатки.ОстатокОстаток КАК Остаток
		|ПОМЕСТИТЬ ВТОстатки
		|ИЗ
		|	РегистрНакопления.ОстаткиМатериалов.Остатки(&ТекущаяДата, Наименование В (&СписокМатериалов)) КАК ОстаткиМатериаловОстатки
		|;
		|ВЫБРАТЬ
		|	ВПроизводствеОбороты.Наименование КАК Наименование,
		|	СУММА(ВПроизводствеОбороты.КоличествоОборот) КАК ВПроизводстве
		|ПОМЕСТИТЬ ВТВПроизводстве
		|ИЗ
		|	РегистрНакопления.ВПроизводстве.Обороты(, , Регистратор, ) КАК ВПроизводствеОбороты
		|ГДЕ
		|	ВПроизводствеОбороты.Регистратор.ДокументОснование.Ссылка = &Ссылка
		|	И ВПроизводствеОбороты.Наименование В (&СписокМатериалов)
		|СГРУППИРОВАТЬ ПО
		|	ВПроизводствеОбороты.Наименование
		|;
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ВТОстатки.Наименование, ВТВПроизводстве.Наименование) КАК Наименование,
		|	ЕСТЬNULL(ВТОстатки.Остаток, 0) КАК Остаток,
		|	ЕСТЬNULL(ВТВПроизводстве.ВПроизводстве, 0) КАК ВПроизводстве
		|ИЗ
		|	ВТОстатки КАК ВТОстатки
		|		ПОЛНОЕ СОЕДИНЕНИЕ ВТВПроизводстве КАК ВТВПроизводстве
		|		ПО ВТОстатки.Наименование = ВТВПроизводстве.Наименование";
	...
	ВыборкаДетальныеЗаписи = РезультатПакета[2].Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Данные = Новый Структура("Остаток, ВПроизводстве",
			ВыборкаДетальныеЗаписи.Остаток,
			ВыборкаДетальныеЗаписи.ВПроизводстве);
		Результат.Вставить(ВыборкаДетальныеЗаписи.Наименование, Данные);
	КонецЦикла;
...
// Возвращает остатки на складе и количество списанного в производство
// для одного материала одним запросом.
//
// Параметры:
//   Ссылка - ДокументСсылка - Ссылка на документ-основание (Рецептуры)
//   Материал - СправочникСсылка.НоменклатураМатериалы - Материал
//
// Возвращаемое значение:
//   Структура - с полями Остаток (Число) и ВПроизводстве (Число)
Функция ОстаткиИВПроизводствеМатериала(Ссылка, Материал) Экспорт
	
	СписокМатериалов = Новый Массив;
	СписокМатериалов.Добавить(Материал);
	
	ДанныеОстатков = ОстаткиИВПроизводстве(Ссылка, СписокМатериалов);
	
	ДанныеМатериала = ДанныеОстатков.Получить(Материал);
	Если ДанныеМатериала <> Неопределено Тогда
		Возврат ДанныеМатериала;
	Иначе
		Возврат Новый Структура("Остаток, ВПроизводстве", 0, 0);
	КонецЕсли;
